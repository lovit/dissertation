...
이 논문은 비지도기반 한국어 자연어처리에 적합한 어절 구조인 L + [R] 의 제안과, 이를 이용하는 네 가지 자연어처리 방법으로 구성되어 있다.
미등록단어 문제를 해결하기 위하여 한국어에 적합한 비지도기반 토크나이저를 제안하며, 이는 언어적 정보를 전혀 이용하지 않는 Word Piece Model 보다도 좋은 성능을, 학습데이터를 이용하는 토크나이저와도 비슷한 단어 인식 성능을 보였다.
둘째, 통계 기반으로 한국어 명사를 추출하는 방법을 제안하며, 이는 학습데이터를 이용하는 기학습된 한국어 형태소 분석기보다도 뛰어난 명사 인식 능력을 보였다.
셋째, 단일 주제로 구성된 한국어 문서 집합을 요약하기 위하여 키워드와 핵심 문장을 선택하는 방법을 제안하며, 이는 단어 추출 능력을 내제하여 토크나이저의 성능에 의존하지 않는다.
또한 중복된 문장을 제거하며 핵심 문장을 선택하는 방법도 제안하였다.
넷째, 다양한 주지로 구성된 문서 집합을 요약하기 위하여 효율적으로 개선한 문서 군집화 방법과 군집화 결과를 바탕으로 작동하는 키워드 추출 방법을 제안하였다.
제안된 군집화 기반 문서 요약 방법은 언어와 상관없이 이용될 수 있는 방법이다.
제안된 방법은 기존의 k-means 알고리즘보다 수백배에서 수천배 빠른 계산 초기화 속도를 보였으며, 추출된 키워드는 군집의 내용이 쉽게 이해될 수 있는 정보력이 큰 단어들을 선택하였다.
...
Natural language processing is a field made up of tasks that convert human language into computer-usable information or solve problems  using it.
Tokenization is the task that recognizes words or morphemes in the given sentences, the results of tokenization is used as input of other natural language processing tasks.
Therefore the poor quality of tokenization degrades the performance of other natural language processing tasks that use the results of tokenizer.

Document summarization is another task that summarizes the contents of a set of documents through keywords or key sentences, and it can be used to facilitate searching for large sets of documents or to index documents.

However, this natural language processing task has the following difficulties.
First, we encounter the problem of unregistered words that do not properly recognize words that do not appear in the learning data, and we call that out-of-vocabulary problem.
Second, it is difficult to prepare training data for each task.
Third, especially, there is a grammar and a spelling error in the text in Korean.
Korean has various errors in text data because readability is not deteriorated even if some spelling errors exist.
And it makes more difficult to recognize words correctly.
These difficulties are associated with each other.
Errors in the data cause an out-of-vocabulary problem, and there is no training data to reduce the errors.
The above difficulties also occur in all other natural language processing tasks including document summarizations.

In this paper, I have proposed unsupervised Korean natural language processing methods to solve the difficults.
Because unsupervised methods rarely depend on training data, it is easy to apply tasks of various domains.
Also, in this paper, I have proposed structure of Eojeol, a word in Korean that used in unsupervised natural language processing tasks.
The structure of Korean helps unsupervised algorithm to learn information efficiently by acting as a prior knowledge.

In this paper, I have proposed L + [R], a structure of Eojeol and four unsupervised natural language processing methods using it.
In order to solve the out of vocabulary problem, I have proposed two unsupervised Korean tokenizers which has better performance than Word Piece Model using any linguistic information.
Additionally, the tokenizers show better or similar performance of recognizing words with supervised Korean tokenizers that uses training data and word dictionary.
Second, I have proposed noun extraction, and it also shows better ability of recognizing nouns than that of trained Korean morpheme analyzers.
Third, I have proposed a seiection methods of keywords and key sentences in order to summarize a set of Korean documents consisting of a single topic.
This method does not depend on the tokenizer and it extracts words itself.
Also, the proposed method have ability to select key sentences while eliminating redundant sentences.
Fourth, to summarize the document set consisting of various topics I have proposed improved document clustering method and effcient keyword extraction method based on clustering result.
The proposed clustering - based document summarization method can be used regardless of language.
The proposed method shows several hundreds to several thousand times faster initialization speed than the existing k-means algorithm, and extracts keywords relevants to cluster summarization.

Many tasks in natural language processing use supervised machine learning algorithms.
Unsupervised methods can be complementary because they use other information that supervised methods do not use.
The proposed unsupervised natural language processing method shows better or similar performance than the supervised ones.
This means that there is a possibility that the performance can be improved by using complementary two approaches.
This paper suggests unsupervised methods that can make it possible.
\end{abstract}
...
Word piece model (WPM)은 재귀적 인공신경망처럼 모델이 학습할 수 있는 단어의 개수가 제한적일 때 사용되는 비지도기반 토크나이저이다 \citep{sennrich2015neural}.
...
\subsubsection{단어 점수를 이용하는 비지도기반 토크나이저}
...
또한 비지도기반 방법은 학습데이터에 대한 의존성이 적기 때문에 독립적으로 학습하는 모델로 발전할 가능성이 크다.
