...
kmeans++ \citep{arthur2007k} 은 이를 위하여 제안된 방법으로, 다음의 과정을 반복하여 초기 군집 대표값을 선택한다.
처음 한 점 $c_0$ 는 임의로 선택한 뒤 이 점을 기준으로 다른 모든 점들과의 거리를 계산하여 그 제곱에 비례한 확률로 다른 점을 선택하여 널리 퍼진 점들을 초기 군집 대표값으로 선택한다.

\begin{enumerate}
  \item Select a point $c_0$ randomly.
  \item Select next point $c_t$ with prob. of $\frac{d(c_{t-1}, c_t)^2}{\sum_t^` d(c_{t-1}, c_{t^`})^2}$
  \item Repeat step 2 until choosing $k$ points
\end{enumerate}
...
큰 크기의 군집이 여러 개로 나뉘어질 경우 후처리로 병합이 가능하지만, 작은 크기의 군집이 여러 개의 큰 군집으로 나뉘어질 경우 이들의 복원은 매우 어렵기 때문이다.
