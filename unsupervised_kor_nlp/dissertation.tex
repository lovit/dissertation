...
첫째, 미등록단어 문제를 해결하기 위하여 제안된 비지도기반 토크나이저는 언어적 정보를 이용하지 않는 Word Piece Model 보다 좋거나 학습데이터를 이용하는 토크나이저와 비슷한 단어 인식 성능을 보였다.
둘째, 통계 기반으로 명사를 추출하는 방법은 학습 말뭉치와 단어 사전을 이용하는형태소 분석기보다도 뛰어난 명사 인식 능력을 보였다.
셋째, 토크나이저에 의존하지 않으며 단어 추출과 함께 키워드를 추출하는 방법을 이용하여 단일 주제로 구성된 한국어 문서 집합을 요약하는 방법을 제안하였다.
이 방법은 특히 키워드의 미등록단어 문제에 강건하며, 키워드 추출 방법을 확장하여 중복되지 않는 핵심문장을 선택하는 방법도 함께 제안하였다.
넷째, 효율적으로 문서 군집화를 수행하고 문서 군집화 결과로부터 각 군집의 키워드를 선택하여 다양한 주제로 구성된 문서 집합을 요약하는 방법을 제안하였다.
...
그러나 음절 단위의 품사 판별은 단어 사전에 존재하지 않는 단어가 발생할 가능성이 있기 때문에 일반적으로 단어 사전 기반의 품사 판별기보다 낮은 성능을 보이는 것으로 알려졌다 \cite{ng2004chinese} \cite{ng2004chinese, shi2007dual, zhang2008joint, lee2016sequence, kakao2018pos}.
...
\subsubsection{키워드 집합을 이용한 핵심문장 선택}
