...
이 논문에서는 비지도학습 한국어 자연어처리에 적합한 어절 구조인 L + [R] 와 이를 이용하는 네 가지 비지도학습 자연어처리 방법을 제안한다.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{제안하는 방법} \label{word_extraction_proposed}
...
\quad $\sigma$ : 사용자에 의하여 지정된 핵심문장간 최소 거리 \\
\quad $k$ : 핵심문장 개수
\quad $KS$ = [ ] : 핵심문장 리스트 \\
\quad $C$ = $dist$($KV, S$) : 핵심문장 비용\\
...
\end{tabular} \\ \hline
\end{tabular}
\caption{KR-WordRank 핵심문장 필터링 함수}
...
\centering
\small
\caption{제안하는 방법 (KR-WordRank), TextRank, 그리고 LexRank 로부터 추출된 핵심문장 예시}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{제안하는 방법: 문서 군집화를 위하여 개선된 Spherical k-means}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{문서 군집화를 위한 효율적인 Spherical k-means 초기화}
...
\subsection{제안된 문서 군집 별 키워드 추출 방법의 성능 평가}
...
Latent Dirichlet Allocationvis 는 토픽 모델링의 결과인 '문서 - 토픽' 확률 행렬과 '토픽 - 단어' 확률 행렬이 주어질 경우, 차원 축소 방법과 키워드 추출 방법을 이용하여 상호형의 시각화된 토픽 모델링 결과를 제공한다 \cite{sievert2014ldavis}.

문서 군집화 결과는 토픽 모델링으로 해석할 수 있다.
Latent Dirichlet Allocation \cite{blei2003latent} 나 pLSI \cite{hofmann1999probabilistic} 는 한 문서에 여러 개의 토픽이 존재할 수 있다고 가정하지만, k-means 는 하나의 문서에 하나의 토픽이 존재한다고 가정한다.
문서 군집화 결과의 '문서 - 토픽' 확률 벡터는 문서 별 토픽에 대한 ont hot 벡터이며, '토픽 - 단어' 확률 행렬은 군집 내 단어의 출현 비율 행렬이다.

Latent Dirichlet Allocationvis 는 각 토픽별로 키워드를 선택하기 위하여 두 종류의 키워드 점수를 조합한다.
...
Latent Dirichlet Allocationvis 와 제안한 키워드 점수를 이용하면 그림 \ref{fig:kmeans_ldavis} 처럼 군집화 결과를 시각화하여 표현할 수 있다.

\begin{figure}[H]
\centering
\includegraphics[keepaspectratio=true, width=0.95\linewidth]{fig/kmeans_visualization.png}
\caption{Latent Dirichlet Allocationvis 를 이용한 spherical k-means 학습 결과 시각화 예시}
...
또한 토픽 모델링의 시각화 방법인 Latent Dirichlet Allocationvis 를 이용하면 spherical k-mean 의 학습 결과도 시각화 할 수 있었다.
